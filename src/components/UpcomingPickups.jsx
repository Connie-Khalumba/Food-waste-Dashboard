...
import { GiTrashCan } from 'react-icons/gi'; // Using react-icons for the trash bin icon
...
const UpcomingPickups = () => {
  ...
  const pickups = [
    { location: 'Subra-face', time: '9:00 am' },
  ...
  return (
    <div className={`bg-white p-4 rounded-lg shadow-md col-span-1 md:col-span-2 ${isDarkMode ? 'dark:bg-gray-800 dark:shadow-gray-700' : ''}`}>
      ...
      <p className={`text-sm ${isDarkMode ? 'dark:text-gray-400' : 'text-gray-500'} mb-2`}>Book your next pickup days</p>
      <ul className="space-y-2">
        {pickups.map((pickup, index) => (
          <li key={index} className="flex items-center space-x-4">
            <GiTrashCan className="w-5 h-5 text-green-500" /> {/* Green trash bin icon */}
