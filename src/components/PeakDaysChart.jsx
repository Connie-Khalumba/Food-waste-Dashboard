...
const PeakDaysChart = () => {
  ...
  return (
    <div className={`bg-white p-2 md:p-4 rounded-lg shadow-md max-h-40 sm:max-h-48 md:max-h-64 h-auto md:h-64 w-full overflow-hidden ${isDarkMode ? 'dark:bg-gray-800 dark:text-white dark:shadow-gray-700' : ''}`}>
      <h2 className={`text-base md:text-lg font-semibold mb-1 md:mb-2 ${isDarkMode ? 'dark:text-white' : ''}`}>Peak Days</h2>
      <p className={`text-xs md:text-sm text-gray-600 mb-1 md:mb-2 ${isDarkMode ? 'dark:text-gray-300' : ''}`}>
        Amount of wasted food given out. <span className={isDarkMode ? 'dark:text-green-400' : 'text-green-600'}>↑ 2.1% vs last week</span>
      </p>
      <p className={`text-xs md:text-sm ${isDarkMode ? 'dark:text-gray-400' : 'text-gray-500'}`}>Report from 1-12 Dec, 2022</p>
